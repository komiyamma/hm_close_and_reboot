...
public partial class Program
{
    ...
    public static async Task Main()
    {
        ...
        // 200ミリ秒単位で、hidemaru というプロセスが存在するか調べる
        for (int count = 0; count < 10; count++)
        {
            var remainProcess = Process.GetProcessesByName(processName);
        ...
        var stillRemainProcess = Process.GetProcessesByName(processName);
        foreach (var p in stillRemainProcess)
        {
            p.Kill();
        }
